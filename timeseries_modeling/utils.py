...
def _create_diff_col(
    df: pd.DataFrame, col: str, as_values: bool = False, lag: int = 1
) -> pd.DataFrame:
...
def adjust_season(
    time_series: pd.DataFrame, period: int, col: str = None
) -> pd.DataFrame:
    """
...
def get_correlation(xs: np.ndarray, ys: np.ndarray) -> np.ndarray:
    ...
    return np.dot(xs_diffs, ys_diffs) / np.sqrt(
        (np.dot(xs_diffs, xs_diffs) * np.dot(ys_diffs, ys_diffs))
    )

...
class TickerDataFrame:
    ...
    def as_df(
        self, start: str = None, end: str = None, period: str = None
    ) -> pd.DataFrame:
    ...
    def time_series(self, start: str, end: str, column: str) -> pd.DataFrame:
        return self.as_df(start=start, end=end).drop(
            columns=[col for col in self.as_df(period="1mo").columns if col != column]
        )

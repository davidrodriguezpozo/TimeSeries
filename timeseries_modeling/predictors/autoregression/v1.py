...
class AutoRegressionPredictor:
    ...
    def linear_regression(
        cls, design_matrix: np.ndarray, target_vector: np.ndarray
    ) -> np.ndarray:
        ...
        """
        return (
            np.linalg.inv(design_matrix.transpose() @ design_matrix)
    ...
    def predict_ar(cls, values: np.ndarray, order: int) -> np.ndarray:
        ...
        for i in range(len(coefficients), len(values)):
            predicted_values.append(
                np.dot(coefficients, values[i - len(coefficients) : i])
            )
        return np.array(predicted_values)

    @classmethod
    def predict(
        cls, df: pd.DataFrame, order: int, col: Optional[str] = None
    ) -> pd.DataFrame:
        df = df.copy()
        col = _get_col(df, col)

...
class GARCHModel:
    ...
    @classmethod
    def compute_arch_predicted_error(
        cls: GARCHModel,
        series: pd.DataFrame,
        col: str = None,
        ar_deg: int = 4,
        ...
        predicted = (
            sm.tsa.AutoReg(series[col].values, lags=ar_deg, trend="n").fit().predict()
        )
        series["Predicted"] = predicted
        error_sq = (series["Predicted"] - series[col]) ** 2
        error_sq[error_sq.isna()] = 0
        error_predicted = (
            sm.tsa.AutoReg(error_sq.values, lags=arch_deg, trend="c").fit().predict()
        )
    ...
    def get_percentages(
        ...
        in_two_sd = (series["Predicted"] + 2 * error_prediction > series[col]) & (
            series["Predicted"] - 2 * error_prediction < series[col]
        )
    ...
    def compute_predicted_error(
        ...
        error_sq[error_sq.isna()] = 0
        error_predicted = (
            sm.tsa.arima.ARIMA(error_sq.values, order=(arch_deg, 0, 3), trend="c")
            .fit()
            .predict()
        )
        series["Error Predicted"] = error_predicted

...
class Decompositions:
    ...
    @classmethod
    def predict(
        cls: Decompositions, series: pd.DataFrame, col: str = None
    ) -> pd.DataFrame:
        ...
        residue_predicted_values = AutoRegressionPredictor.predict_ar(
            decomposed_df["Res diff"].values, 12
        )
        residue_prediction = (
            residue_predicted_values + decomposed_df["Residual"].shift()
        )
    ...
    def predict_lowess(
        ...
        series = series.copy()
        col = _get_col(series, col)
        series["smoothed"] = sm.nonparametric.lowess(
            series["value"], series.index, frac=0.03
        )[:, 1]
    ...
    @classmethod
    def predict_STL(
        cls: Decompositions, series: pd.DataFrame, col: str = None, period: int = None
    ) -> pd.DataFrame:
        col = _get_col(series, col)
